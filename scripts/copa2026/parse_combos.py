"""Parse the 495-combination FIFA RO32 third-place assignment table."""
import sys, json
sys.stdout.reconfigure(encoding="utf-8")

SLOTS = ["1A","1B","1D","1E","1G","1I","1K","1L"]

# RAW = tab-separated lines: row# | col_A..col_L (blank or letter) | 3X*8
# Each line represents one of the 495 C(12,8) combinations
ROWS = [
    ...
    (26, "CDEGHIJKL"[:-1], ["E","G","I","C","J","D","L","K"]),  # CDEGHIJK -> 9 chars, wrong
    (26, "CDEGIJKL", ["E","G","I","C","J","D","L","K"]),
    (27, "CDEGHIJKL"[:-1], ["E","G","J","C","H","D","L","K"]),
    ...
    (63, "BDEGHIJKL"[:-1], ["E","J","B","D","H","G","L","K"]),
    ...
    (91, "BCEGHIJKL"[:-1], ["E","J","B","C","H","G","L","K"]),
    ...
    (141,"BCDEGHIJL"[:-1], ["H","G","B","C","J","D","L","E"]),
    ...
    (146,"BCDEFIJKL"[:-1], ["C","J","B","D","E","F","L","K"]),
    ...
    (156,"BCDEFGIJKL"[:-2], ["C","G","B","D","E","F","L","K"]),
    ...
    (183,"ADEGHIJKL"[:-1], ["E","G","J","D","A","H","L","K"]),
    ...
    (211,"ACEGHIJKL"[:-1], ["E","G","J","C","A","H","L","K"]),
    ...
    (256,"ACDEGIJKL"[:-1], ["E","G","J","C","A","D","L","K"]),
    ...
    (261,"ACDEGHIJL"[:-1], ["H","G","J","C","A","D","L","E"]),
    ...
    (295,"ABEGHIJKL"[:-1], ["E","J","B","A","H","G","L","K"]),
    ...
    (335,"ABDEHIJKL"[:-1], ["E","J","B","A","I","D","L","K"]),
    ...
    (340,"ABDEGIJKL"[:-1], ["E","J","B","D","A","G","L","K"]),
    ...
    (345,"ABDEGHIJL"[:-1], ["H","J","B","D","A","G","L","E"]),
    ...
    (461,"ABCDEIJKL"[:-1], ["E","J","B","C","A","D","L","K"]),
    ...
    (477,"ABCDEGH", ["H","G","B","C","A","D","L","E"]),  # only 7 groups - placeholder
    (477,"ABCDEGL", ["H","G","B","C","A","D","L","E"]),  # placeholder
...
skipped = 0
for entry in ROWS:
    row_num, key_raw, assigns = entry
    # Clean up key - ensure exactly 8 chars from A-L
    key = "".join(sorted(set(c for c in key_raw if c in "ABCDEFGHIJKL")))
    if len(key) != 8:
        skipped += 1
        continue
    if key in seen_keys:
        continue
    seen_keys.add(key)
    assignment = {slot: g for slot, g in zip(SLOTS, assigns)}
    combos[key] = assignment

print(f"Parsed: {len(combos)} / 495 combinations  |  Skipped/duplicate: {skipped}")
print("EFGHIJKL ->", combos.get("EFGHIJKL"))  # Expected: 1A=E,1B=J,1D=I,1E=F,1G=H,1I=G,1K=L,1L=K
print("DFGHIJKL ->", combos.get("DFGHIJKL"))  # Expected: 1A=H,1B=G,1D=I,1E=D,1G=J,1I=F,1K=L,1L=K
print("ABCDEFGH ->", combos.get("ABCDEFGH"))  # Row 495: 1A=H,1B=G,1D=B,1E=C,1G=A,1I=F,1K=D,1L=E

with open("combos_temp.json", "w", encoding="utf-8") as f:
    json.dump(combos, f, ensure_ascii=False)
print("Saved combos_temp.json")
