---
title: Club League Simulator — Blueprint
type: project-blueprint
status: design-complete, pre-implementation
created: 2026-06-17
tags: [football-vault, simulation, dixon-coles, monte-carlo, portfolio]
related: [football-vault, predictor-wc]
---

# Club League Simulator — Blueprint
...
## 2. Diferença em relação ao predictor-wc

| | predictor-wc | club-league-simulator |
|---|---|---|
...
Modelo estatístico (Dixon-Coles, Poisson, Elo) é compartilhado em espírito entre os dois projetos; vale avaliar reuso de código entre eles no futuro.
...
|---|---|---|
| 2010s–hoje | Elenco completo, minutos, gols, assistências, às vezes xG | Atribuição de jogador completa (ver seção 5) |
| 1990s–2000s | Elenco e gols geralmente disponíveis, minutos esparsos | Atribuição por gols/jogos disputados, sem minutos |
| 1970s–1980s | Gols artilheiros documentados, elenco incompleto | Atribuição só para artilheiros conhecidos + "outros" agrupado |
| 1930s–1960s | Só placar, eventualmente lista de quem marcou | Sem atribuição de jogador — fica só no nível de time |
...
|---|---|---|---|
| 2000s+ | Completo | Gols, assistências, geralmente minutos | Ponderação por taxa (gols/90min) |
| 1990s | Completo | Gols geralmente sim, assistências inconsistentes, minutos raros | Ponderação por gols/jogos disputados |
| 1980s e antes | Completo | Gols disponíveis, assistências praticamente inexistentes | Atribuição de gol; assistência cai para categoria "não atribuída" |
...
**Nota de acesso:** fetch direto ao Transfermarkt está bloqueado (SITE_BLOCKED via ferramentas automatizadas). Para coleta em escala, será necessário avaliar API oficial/parceira, datasets já compilados (Kaggle), ou outra estratégia de coleta — respeitando termos de uso do site. Não resolvido ainda.

---

## 4. Motor de resultado (Camada 0)

Foco declarado: Dixon-Coles + Rue-Salvesen, não dependência pesada de janela de anos vizinhos para estimar "nível de gols da época" — clubes têm volume suficiente de jogos na própria temporada-base para isso. A janela de anos serve para reduzir ruído (times com poucos jogos, ex: promovidos), não para definir o nível médio de gols.
...
### Formato: Parquet
Colunar, comprimido — 380 jogos × 1.000 iterações = 380.000 linhas cabem em poucos MB. Compatível com leitura client-side via DuckDB-WASM (ou similar), permitindo filtragem direto no navegador sem backend — essencial para o requisito de hospedagem 100% estática ("sem PC ligado").
...
├── Top scorers evolving             [toggle: simulated / real]
...
└── Ao abrir uma:
    ├── Reabrir Live Sim do zero (replay completo), ou
    ├── Ir direto ao Recap, ou
    └── Ir à Distribution View
```
...
Objetivo: www.hmantovani.com, **sem servidor rodando continuamente**.
...
- [ ] Resolver estratégia de coleta de dados do Transfermarkt em escala (fetch direto bloqueado; avaliar API oficial, datasets Kaggle, ou outra fonte compatível com termos de uso)
...
- [ ] Definir critério de desempate na tabela de classificação (afeta `calcular_tabela`)
- [ ] Avaliar reuso de código de modelo (Dixon-Coles) entre este projeto e o `predictor-wc`

---

## 12. Relação com outros projetos do vault

- **predictor-wc**: projeto irmão, prospectivo em vez de retrospectivo. Compartilha base estatística (Poisson/Elo/Dixon-Coles), mas com propósito e fluxo de dados opostos. Avaliar componentização futura.
- **football-vault**: este documento deve ser referenciado como um dos sistemas preditivos/de simulação dentro do blueprint mestre do Football Vault, particularmente nas seções de "sistemas preditivos" e "infraestrutura de dados por entidade (jogador/clube)".
